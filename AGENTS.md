...
- 06:30 首次生成，06:50 和 07:10 仅重试失败或缺失频道。
- 前两次生成失败只写运行台账，不发送频道失败卡片。
- 07:10 最终生成重试后，仍失败的频道才发送一次异常卡片。
- 09:01 至 09:51 每十分钟执行发布检查；10:05 执行宽限重试。
- 10:05 后仍未通过公网验证的频道才发送发布异常卡片。
- 同一日期、频道和故障类型最多通知一次。
- 模型用量只在最终生成重试后检查一次；单频道超过保护阈值，或 token 超过有效历史基线两倍才告警。
