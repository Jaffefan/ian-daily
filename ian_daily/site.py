...
.player-main{grid-template-columns:42px minmax(0,1fr) auto auto 42px 110px 42px}.lucide{width:19px;height:19px;display:block;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.download{width:42px;height:42px;border:1px solid #60645f;display:grid;place-items:center}.chapter-drawer summary{display:none}.source-list{margin-top:12px;padding:10px 14px;border-left:2px solid var(--channel);background:#f6f4ed}.source-list a{overflow-wrap:anywhere}.mute{width:42px;height:42px}
@media(max-width:760px){.player{padding:9px 0}.player-main{grid-template-columns:40px minmax(0,1fr) auto auto;gap:8px}.wave{height:36px}.time{grid-column:2;font-size:11px}.speed{grid-column:3;grid-row:2;padding:5px}.mute{grid-column:4;grid-row:1}.volume,.download{display:none}.chapter-drawer summary{display:flex;align-items:center;gap:7px;color:#d8d9d4;font-size:12px;cursor:pointer;padding-top:7px;list-style:none}.chapter-drawer summary::-webkit-details-marker{display:none}.chapters{padding-top:7px}.chapter{padding:6px 9px;font-size:12px}.player-shell{max-height:190px}.story{scroll-margin-top:205px}}
"""
...
def _icon(name: str) -> str:
    paths = {
        ...
        "muted": '<polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="22" x2="16" y1="9" y2="15"/><line x1="16" x2="22" y1="9" y2="15"/>',
        "download": '<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/>',
        "chapters": '<path d="M4 6h16M4 12h16M4 18h10"/>',
    }
    return f'<svg class="lucide" viewBox="0 0 24 24" aria-hidden="true">{paths[name]}</svg>'
...
def _episode_page(bundle: EpisodeBundle, report: QualityReport, previous: EpisodeBundle | None, following: EpisodeBundle | None) -> str:
    ...
    for index, section in enumerate(bundle.reading.sections, 1):
        origin = f' · <a href="{_e(section.image_source_url)}" target="_blank" rel="noopener">查看原图</a>' if section.image_source_url else ""
        image = f'<img class="story-image" src="{_e(section.image_url)}" alt="{_e(section.title)}"><div class="credit">图片来源：{_e(section.image_credit)}{origin}</div>'
    ...
    cover_source = bundle.reading.sections[0].image_source_url if bundle.reading.sections else ""
    cover_origin = f' · <a href="{_e(cover_source)}" target="_blank" rel="noopener">查看原图</a>' if cover_source else ""
    ...
    body = f'''<main class="article"><section class="article-hero"><div class="channel-name">伊恩每日 · {_e(bundle.category_name)}</div><h1>{_e(bundle.podcast.title)}</h1><p class="article-lead">{_e(bundle.reading.lead or bundle.podcast.description)}</p><div class="eyebrow">{_e(bundle.date_bjt)} · {report.story_count} 个事件 · {report.audio_duration_sec / 60:.1f} 分钟</div><img class="hero-cover" src="{_e(cover)}" alt="节目封面"><div class="hero-credit">图片来源：{_e(credit)}</div></section></main><section class="player-shell"><div class="player"><audio id="audio" preload="metadata" src="episode.mp3"></audio><div class="player-main"><button class="icon-button" id="play" aria-label="播放">{_icon('play')}</button><canvas class="wave" id="wave" aria-label="音频时间轴" role="slider" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"></canvas><span class="time" id="time">00:00 / 00:00</span><select class="speed" id="speed" aria-label="播放速度"><option value="1">1×</option><option value="1.25">1.25×</option><option value="1.5">1.5×</option><option value="2">2×</option></select><button class="icon-button mute" id="mute" aria-label="静音">{_icon('volume')}</button><input class="volume" id="volume" type="range" min="0" max="1" step="0.05" value="1" aria-label="音量"><a class="download" href="episode.mp3" download aria-label="下载音频">{_icon('download')}</a></div><details class="chapter-drawer" id="chapter-drawer"><summary>{_icon('chapters')}章节</summary><div class="chapters">{buttons}</div></details></div></section><main class="article">{''.join(stories)}<section class="transcript"><details><summary>无障碍文字记录</summary>{transcript}</details></section><nav class="episode-nav">{previous_link}<a class="back" href="../">返回{_e(bundle.category_name)}频道</a>{next_link}</nav></main><script>
const A=document.querySelector('#audio'),P=document.querySelector('#play'),M=document.querySelector('#mute'),W=document.querySelector('#wave'),T=document.querySelector('#time'),S=document.querySelector('#speed'),V=document.querySelector('#volume'),D=document.querySelector('#chapter-drawer'),C=[...document.querySelectorAll('.chapter')],H=[...document.querySelectorAll('.listen-here')],PEAKS={peaks},KEY='ian-daily:'+location.pathname,PLAY={json.dumps(_icon('play'))},PAUSE={json.dumps(_icon('pause'))},VOLUME={json.dumps(_icon('volume'))},MUTED={json.dumps(_icon('muted'))};const fmt=s=>`${{Math.floor(s/60).toString().padStart(2,'0')}}:${{Math.floor(s%60).toString().padStart(2,'0')}}`;function draw(){{const d=devicePixelRatio||1,w=W.clientWidth,h=W.clientHeight;W.width=w*d;W.height=h*d;const x=W.getContext('2d');x.scale(d,d);x.clearRect(0,0,w,h);const progress=A.duration?A.currentTime/A.duration:0;W.setAttribute('aria-valuenow',Math.round(progress*100));PEAKS.forEach((p,i)=>{{const px=i*w/PEAKS.length,bh=Math.max(2,p*h*.9);x.fillStyle=i/PEAKS.length<=progress?'{_color(bundle.category)}':'#676b66';x.fillRect(px,(h-bh)/2,Math.max(1,w/PEAKS.length-1),bh)}})}}function active(){{let index=0;C.forEach((b,i)=>{{if(+b.dataset.start<=A.currentTime)index=i;b.classList.toggle('active',i===index)}})}}function drawer(){{if(innerWidth>760)D.open=true}}A.addEventListener('loadedmetadata',()=>{{const saved=+localStorage.getItem(KEY)||0;if(saved<A.duration-10)A.currentTime=saved;T.textContent=fmt(A.currentTime)+' / '+fmt(A.duration);draw()}});P.onclick=()=>A.paused?A.play():A.pause();A.onplay=()=>P.innerHTML=PAUSE;A.onpause=()=>P.innerHTML=PLAY;A.ontimeupdate=()=>{{T.textContent=fmt(A.currentTime)+' / '+fmt(A.duration||0);localStorage.setItem(KEY,A.currentTime);active();draw()}};W.onclick=e=>{{if(A.duration)A.currentTime=(e.offsetX/W.clientWidth)*A.duration}};S.onchange=()=>A.playbackRate=+S.value;V.oninput=()=>{{A.volume=+V.value;A.muted=false;M.innerHTML=VOLUME}};M.onclick=()=>{{A.muted=!A.muted;M.innerHTML=A.muted?MUTED:VOLUME}};C.forEach(b=>b.onclick=()=>{{A.currentTime=+b.dataset.start;A.play();const id=b.dataset.story;if(id)document.querySelector('#story-'+id)?.scrollIntoView({{behavior:'smooth',block:'start'}})}});H.forEach(b=>b.onclick=()=>{{A.currentTime=+b.dataset.start;A.play()}});document.addEventListener('keydown',e=>{{if(e.target.matches('input,select,button,a'))return;if(e.code==='Space'){{e.preventDefault();P.click()}}if(e.code==='ArrowRight')A.currentTime=Math.min(A.duration||0,A.currentTime+10);if(e.code==='ArrowLeft')A.currentTime=Math.max(0,A.currentTime-10)}});drawer();addEventListener('resize',()=>{{drawer();draw()}});
</script>'''
...
def build_site(store: EpisodeStore | None = None, include_ids: set[str] | None = None) -> Path:
    ...
    for bundle in bundles:
        ...
        needs_images = any(
            ...
            or section.image_kind == "fallback"
